...
const DIFFUSION: [u32; 32] = [0xf26cb481,0x16a5dc92,0x3c5ba924,0x79b65248,0x2fc64b18,0x615acd29,0xc3b59a42,0x976b2584,
    0x6cf281b4,0xa51692dc,0x5b3c24a9,0xb6794852,0xc62f184b,0x5a6129cd,0xb5c3429a,0x6b978425,
    0xb481f26c,0xdc9216a5,0xa9243c5b,0x524879b6,0x4b182fc6,0xcd29615a,0x9a42c3b5,0x2584976b,
    0x81b46cf2,0x92dca516,0x24a95b3c,0x4852b679,0x184bc62f,0x29cd5a61,0x429ab5c3,0x84256b97];
const CONFUSION: [u8; 512] =
...
fn forward(input: &mut [u8;32], output: &mut [u8;32], confusion: &[u8;512], diffusion: &[u32; 32], rounds: usize) {
    for _ in 0..rounds {
        // Stage 1
        for j in 0..32_usize {
        ...
        for j in 0..32_usize {
            for k in 0..32_usize {
                input[j] ^= (output[k] as u32 * ((diffusion[j] >> k) & 1)) as u8;
            }
        }
    }
    // Stage 3
    for i in 0..16_usize {
        output[i] = confusion[input[i * 2] as usize] ^ confusion[input[i * 2 + 1] as usize + 256_usize];
    }
}

fn convert_solution(solution: &Vec<u8>) -> [u8; 32] {
    let mut res = [0;32];
...
fn main() {
    ...
    target[0..15].copy_from_slice("Hire me!!!!!!!!".as_bytes());

    let cracks = cracker::crack(&target, &DIFFUSION, &CONFUSION, 256);
    for (idx, crack) in cracks.iter().enumerate() {
        println!("Crack {}: {:?}", idx, crack);
    }
    let mut input: [u8; 32] = convert_solution(&cracks[0]);
    let mut output: [u8; 32] = [0; 32];
    forward(&mut input, &mut output, &CONFUSION, &DIFFUSION, 256);
    println!("Output: {:?}", output);
    println!("Target: {:?}", target);

    // println!("{:?}", cracker::xor_match(&CONFUSION, 0));

    // let mut matrix = cracker::compute_matrix(&DIFFUSION);
    // let inv = cracker::compute_inverse(&matrix);
    // for row in inv.iter() {
    //     println!("{:?}", row);
    // }
    // println!("{:?}", cracker::matrix_mult(&inv, &output.to_vec()));
    // println!("{:?}", cracker::reverse_stage3(&CONFUSION, &target))
    // println!("{:?}", cracker::build_lookup_table(&CONFUSION));
}
